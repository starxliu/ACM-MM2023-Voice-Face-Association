    ...
    def getFeat(self, save_path):
        ...
        for i in tqdm(range(self.num_id)):
            ...
            with torch.no_grad():
                ...
                auds = auds.cuda()
                img_feats = self.model.vNet(imgs)
                aud_feats = self.model.aNet(auds)
            ...
            for j in range(len(aud_feats)):
                # tems = aud_filenames[j].split('/')
                # tems[0] = self.test_set.audio_name2id[tems[0]]
                # aud_filenames[j] = '/'.join(tems)
        ...
        torch.save(save, save_path, _use_new_zipfile_serialization=False)
        print('Done.')

    def load_feat(self):
        save_path = self.args.evaluation.save_feat
        if not osp.exists(save_path):
            self.getFeat(save_path)
    ...
    def eval(self):
        ...
        with open(self.args.evaluation.save_result, 'w') as f:
            for i in range(len(all_res)):
                f.write(args.eval_lists[i] + ' ' + str(all_res[i]) + '\n')
