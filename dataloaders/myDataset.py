...
class MyDataset(BaseDataset):
    def __init__(self, args, split='train', sample_mode='list', **kwargs):
        ...
        def get_all_files(dir, ext):
            ...
            ret = []
            for i in file_list:
                if (not get_all_files(os.path.join(dir, i), ext)):
                    pass
                else:
                    ret += get_all_files(os.path.join(dir, i), ext)
            return ret
        ...
        ## audio_name2id mapping
        self.audio_name2id = dict()
        ...
        with open(_data_list) as f:
            ...
            for line in lines:
                ...
                self.audio_name2id[face_id] = audio_id

                line['image_list'] = get_all_files(osp.join(self.root_dir_image, face_id), ['.jpg', '.png'])
                line['audio_list'] = get_all_files(osp.join(self.root_dir_audio, face_id), ['.npy', '.wav'])
        ...
        for i in range(len(self.metas)):
            metas_index = self.id_mapping[self.metas[i]['VGGFace1_ID']]
            self.sbs[metas_index] = [np.random.choice(range(len(self.metas[i]['image_list']))), np.random.choice(range(len(self.metas[i]['audio_list'])))]
        for i in range(len(self.metas)):
            metas_index = self.id_mapping[self.metas[i]['VGGFace1_ID']]
            self.sbs2[metas_index] = [np.random.choice(range(len(self.metas[i]['image_list']))), np.random.choice(range(len(self.metas[i]['audio_list'])))]
