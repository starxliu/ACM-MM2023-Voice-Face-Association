...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        for key in key_list:
            ...
            elif 'audio' == key or 'audio_aux' == key:
                aud = sample[key]
...
class RandomHorizontalFlip(object):
    def __call__(self, sample):
        if np.random.rand() < 0.5:
            ...
            for key in key_list:
                if 'image' in key or 'image_aux' in key:
                    image = sample[key]
...
class Resize(object):
    ...
    def __call__(self,sample):
        ...
        key_list = sample.keys()
        for key in key_list:
            if key != 'image' and key != 'image_aux':
                continue
            img = sample[key]
            h, w = img.shape[:2]
            img = cv2.resize(img, dsize=(out_w,out_h), interpolation=cv2.INTER_LINEAR if 'image' in key else self.seg_interpolation)
            img = cv2.resize(img, dsize=(self.output_size[0], self.output_size[1]), interpolation=cv2.INTER_LINEAR if 'image' in key else self.seg_interpolation)
            sample[key] = img
...
class ColorJitter(object):
    def __init__(self, brightness=0.2, contrast=0.2, saturation=0.3, hue=0.3):
        self.colorjitter = transforms.ColorJitter(brightness=0.0, contrast=0.0, saturation=0.0, hue=0.2)
    ...
    def __call__(self, sample):
        ...
        for key in key_list:
            if 'image' not in key:
                continue
            img = sample[key]
            sample[key] = self.colorjitter(img)
